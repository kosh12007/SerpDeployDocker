...
Перед запуском скрипта ваш домен должен указывать на IP-адрес этого сервера. Зайдите в панель управления вашего регистратора (где вы купили домен) и создайте **ДВЕ A-записи**:

1. **Тип:** `A` | **Имя/Хост:** `@` (или пусто) | **Значение/IP:** `IP_ВАШЕГО_СЕРВЕРА`
2. **Тип:** `A` | **Имя/Хост:** `www` | **Значение/IP:** `IP_ВАШЕГО_СЕРВЕРА`

*(Обновление DNS может занять от 15 минут до нескольких часов. Проверить привязку можно командой `ping вашдомен.ru`).*

### Шаг 4: Автоматический запуск и получение SSL

Мы подготовили специальный скрипт `init-vps.sh`, который за одну команду запустит сервер, получит сертификаты и настроит Nginx.
