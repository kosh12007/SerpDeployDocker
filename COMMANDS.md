...
## 📤 Сохранение изменений в репозиторий (Commit + Push)
Когда вы изменили файлы на вашем компьютере (в Visual Studio Code) и хотите отправить их на GitHub, чтобы затем скачать на сервер:
```bash
# 1. Добавить все измененные файлы
git add .

# 2. Создать слепок изменений (внутри кавычек опишите, что изменили)
